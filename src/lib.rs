...
impl CalendarSyncService {
    pub async fn request_access() -> Result<bool, CalendarSyncServiceError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let completion_handler = RcBlock::new(move |granted: Bool, error: *mut NSError| {
                if let Some(tx) = transmitter_clone.lock().unwrap().take() {
                    if !error.is_null() {
                        ...
                        let _ = tx.send(Err(if error_info.code == EKErrorCode::EKErrorEventStoreNotAuthorized.0 {
            ...
            let block_ptr = &*completion_handler as *const Block<(dyn Fn(Bool, *mut NSError))> as *mut Block<(dyn Fn(Bool, *mut NSError))>;
            unsafe {
                event_store.requestFullAccessToEventsWithCompletion(block_ptr);
            }
        });
    
        receiver.await.map_err(|_| CalendarSyncServiceError::ChannelError)?
    }
}
...
mod tests {
    ...
    async fn test_calendar_access_request() {
        ...
        assert!(result.is_ok(), "カレンダーアクセス権限のリクエストに失敗: {:?}", result);
    }
} 